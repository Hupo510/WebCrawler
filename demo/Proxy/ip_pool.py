    ...
    * storage
        ...
        try:
            ...
            if len(urls) == len(ports):
                for i in range(len(urls)):
                    ...
                    sys.stdout.write('\r' + full_ip + ' ' * (22 - len(full_ip))
                                     + '<' + str(self.ip_num) + '>')
                    sys.stdout.flush()
    ...
    def get_all_ip(self):
        ...
        for i in range(self.page_num):
            ...
            results = self.get_content(
                url_xici, url_xpath_xici, port_xpath_xici)
            self.all_ip.update(results)
            current_all_ip.update(results)
            time.sleep(0.5)
        ...
        for i in range(self.page_num):
            ...
            results = self.get_content(
                url_mimi, url_xpath_mimi, port_xpath_mimi)
            self.all_ip.update(results)
            current_all_ip.update(results)
            time.sleep(0.5)
        ...
        for i in range(self.page_num):
            url_kuaidaili = 'http://www.kuaidaili.com/free/inha/' + \
                str(i + 1) + '/'
            results = self.get_content(
                url_kuaidaili, url_xpath_kuaidaili, port_xpath_kuaidaili)
            self.all_ip.update(results)
            current_all_ip.update(results)
            time.sleep(0.5)
        ...
        for p in ip_set:
            ...
            try:
                ...
                if r.status_code == 200:
                    print('succeed: ' + p + '\t' + " succeed in " +
                          format(end - start, '0.4f') + 's!')
                    # add to result
    ...
    def multi_thread_validation(self, ip_set, manager_list, timeout, thread=50):
        ...
        for i in range(thread - 1):
            ...
            ip_set -= part
            p = Process(target=self.get_valid_ip,
                        args=(part, manager_list, timeout))
            jobs.append(p)
            p.start()

        # the last slice of ip_set.
        p = Process(target=self.get_valid_ip, args=(
            ip_set, manager_list, timeout))
        p.start()
        jobs.append(p)
    ...
    def save_to_db(self, valid_ips):
        ...
        try:
            for item in valid_ips:
                ...
                item_exist = cursor.execute(
                    'SELECT * FROM %s WHERE content="%s"' % (cfg.TABLE_NAME, item))

                # it's new ip
                if item_exist == 0:
                    n = cursor.execute(
                        'INSERT INTO %s VALUES("%s", 1, 0, 0, 1.0, 2.5)' % (cfg.TABLE_NAME, item))
                    conn.commit()

                    if n:
                        print(datetime.datetime.now().strftime(
                            "%Y-%m-%d %H:%M:%S") + " " + item + " insert successfully.")
                    else:
                        print(datetime.datetime.now().strftime(
                            "%Y-%m-%d %H:%M:%S") + " " + item + " insert failed.")

                else:
                    print(datetime.datetime.now().strftime(
                        "%Y-%m-%d %H:%M:%S") + " " + item + " exists.")
        except Exception as e:
    ...
    def get_proxies(self, manager_list):
        ...
        try:
            ...
            if len(result):
                ...
                self.multi_thread_validation(
                    current_ips, manager_list, cfg.timeout)
